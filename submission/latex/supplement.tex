...
\section{Sampling design: surrogate and the continuum certificate}

\paragraph{Relation to classical design.}  Write $\Psimat=\Phimat_{\Omega}$ for the atoms
of the candidate out-of-band set $\Omega$.  The design goal---keep
$\tfrac1N\Phimat^{*}\Phimat$ near $\bm I$ while driving $\tfrac1N\Phimat^{*}\Psimat$
toward $0$, indifferent to $\tfrac1N\Psimat^{*}\Psimat$---is the $D_s$-optimality
(interest-vs-nuisance) criterion of experimental design and LCMV/MVDR null-steering: the
information about the interest block is the Schur complement
$J_{\Lambda\Lambda}-J_{\Lambda\Omega}J_{\Omega\Omega}^{-1}J_{\Omega\Lambda}$, which equals
the isolated interest block iff the cross-block $J_{\Lambda\Omega}=\Phimat^{*}\Psimat$
vanishes.  We do not claim this principle; our additions are the identifiability reading
(Thm.~1 says grid designs \emph{cannot} realize it for coherent folds) and the certificate
below.
...
\begin{proposition}[continuum certificate; Prop.~\ref{prop:cert-num} in the paper]
\label{prop:cert-num}
Fix $T$ with $\sigma_{\min}(\Phimat)>0$ and let
$\bm M=\Phimat(\Phimat^{*}\Phimat)^{-2}\Phimat^{*}$ (Hermitian PSD).  Then
$g(\nu):=\at(\nu)^2=\bm\phi_\nu^{*}\bm M\bm\phi_\nu
=\sum_{j,l}\bm M_{jl}\,e^{i2\pi\nu(t_l-t_j)}$ is a real trigonometric polynomial in $\nu$,
$L$-Lipschitz with $L=2\pi\sum_{j,l}|\bm M_{jl}|\,|t_l-t_j|$.  Consequently, for any
interval $[a,b]$ and any grid on it of spacing $h$,
\[
...
\begin{proof}
$\bm\phi_\nu[j]=e^{i2\pi\nu t_j}$, so $\bm\phi_\nu^{*}\bm M\bm\phi_\nu=\sum_{j,l}\bm M_{jl}
e^{-i2\pi\nu t_j}e^{i2\pi\nu t_l}$, real because $\bm M$ is Hermitian.  Its derivative is
$g'(\nu)=\sum_{j,l}\bm M_{jl}\,i2\pi(t_l-t_j)e^{i2\pi\nu(t_l-t_j)}$, so
$|g'|\le2\pi\sum_{j,l}|\bm M_{jl}||t_l-t_j|=L$ (a Bernstein-type bound; each exponential is
unit-modulus).  On a grid of spacing $h$, any point is within $h/2$ of a grid node, so
$g(\nu)\le\max_{\text{grid}}g+L(h/2)$; take the sup and the square root (monotone).  The
visibility bound is identical with $\bm P$ and $\vt(\nu)^2=1-h(\nu)/N$.
\end{proof}
Everything in $\bm M,\bm P,L$ depends only on the sample times, so the certificate is
sample-only; it is validated in \texttt{tests/test\_design.py::test\_certificate\_is\_sound}
(certified bound $\ge$ true value on a fine reference grid, and tight: the Lipschitz slack
is negligible at the grid resolutions used).  This is exactly the guarantee the
finite-candidate bound of Theorem~3(b) cannot give, and for which random designs have no
analogue.
